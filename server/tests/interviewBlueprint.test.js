...
test('target question counts stay within 20-25 and fullstack is broader', () => {
...
test('remaining topics shrink as turns cover a role', () => {
  const turns = [
...
test('fallback topics prioritize uncovered role topics', () => {
...
test('question style rotates through theory, scenario, and practical', () => {
...
test('remaining estimate never drops below zero', () => {
  const remaining = estimateRemainingQuestions([{ topic: 'x' }], 1)
  assert.equal(remaining, 0)
})

test('frontend and backend role guardrails detect blocked keywords', () => {
